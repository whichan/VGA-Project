...
`timescale 1ns / 1ps
//=============================================================================
// ImgMemReader — 업스케일 보간 + 바운딩 박스 오버레이
//
// 기능:
//   1. 320×240 → 640×480 업스케일 (가로 바이리니어 보간)
//      짝수 x: 원본 픽셀 그대로
//      홀수 x: 좌우 원본 픽셀의 평균값 (부드러운 전환)
//      세로: 2배 복사
...
module ImgMemReader #(
  ...
  // =========================================================================
  // QVGA 좌표 변환: VGA(640×480) → QVGA(320×240)
  // =========================================================================
  wire [8:0] qvga_x = x_pixel[9:1];  // /2 → 0~319
  wire [7:0] qvga_y = y_pixel[9:1];  // /2 → 0~239

  // =========================================================================
  // BRAM 주소 생성 (보간용)
  //   짝수 x → 현재 원본 픽셀 주소 (qvga_x)
  //   홀수 x → 다음 원본 픽셀 주소 (qvga_x + 1)
  //   이전 원본 픽셀은 레지스터에 저장
  // =========================================================================
  wire [8:0] read_x = x_pixel[0] ? (qvga_x + 1) : qvga_x;
  ...
  // =========================================================================
  // 바운딩 박스 테두리 (VGA 좌표 기준, ×2 스케일)
  // =========================================================================
  wire [9:0] bx_min = {box_x_min, 1'b0};  // QVGA → VGA ×2
